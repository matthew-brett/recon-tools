...
def solve_params(image, pct=75.0, cutn1=True, smooth=False, window=False):
    ...
    phs_mean, q1_mask = UBPC_dev.mean_and_mask(p_evn, p_odd,
                                               pct*pct_scale, N.arange(Q3))

    a1,a3,a0 = UBPC_dev.solve_phase_3d(phs_mean, q1_mask)
    return (a1,a3,a0)
...
def solve_params_epi(epi, svdsize=3, pct=25., cutn1=True, smooth=False):
    ...
    neg_order = 1
    phs_vol = util.unwrap_ref_volume(inv_ref)
    phs_mean, q1_mask = UBPC_dev.mean_and_mask(phs_vol[:,pos_order,:],
                                               phs_vol[:,neg_order,:],
                                               pct*pct_scale,
                                               N.arange(refShape[-3]))
    if svdsize == 3:
        coefs = UBPC_dev.solve_phase_3d(phs_mean, q1_mask)
    else:
        coefs = UBPC_dev.solve_phase_6d(phs_mean, q1_mask)
    return coefs
