...
def dtypes_test(image, iomodule, output_type):
    ...
    *output_type is the string identifier for the output (eg 'analyze')
    ...
    image_max = N.abs(image[:]).max()
    image_min = N.abs(image[:]).min()
    ...
    for datatype, dt in imageio.recon_output2dtype.items():
        ...
        im_max2 = N.abs(image2[:]).max()
        im_min2 = N.abs(image2[:]).min()
...
class test_nifti(NumpyTestCase):
    ...
    def test_datatypes(self):
        ...
        image = self.image._subimage(1949.*N.random.rand(*self.image.shape) +
                                     1.j*5821*N.random.rand(*self.image.shape))
