...
#*********************************
def parse_procpar(filename,verbose):
#*********************************

#   Parse procpar and return parameter values.
#   verbose=1: print results to stdout.
    ...
    while 1:
        ...
        if (keywords[0] == 'thk'):
            ...
            if verbose:
                print "thk %s" % (thk)
            params['thk'] = thk
        elif (keywords[0] == 'flash_converted'):
            flash_converted = "1"
        elif (keywords[0] + " 1" == "theta 1"):
            line = infile.readline()
            words = string.split(line)
            theta = words[1]
            if verbose:
...
#            params['gap'] = gap
            ...
            pss = words[1:]
            if(verbose):
                print "nslice: %s" % nslice
                for xpss in pss:
                    print "pss %s" % (xpss)
            params['pss'] = pss
            params['nslice'] = nslice
        elif (keywords[0] == 'asym_time'):
            line = infile.readline()
            words = string.split(line)
            nvol = words[0]
            params['nvol'] = nvol
            ...
            if(verbose):
                print "nvol: %s" % nvol
                for xasym_times in asym_times:
                    print "asym_times %s" % (xasym_times)
            if len > 1:
                params['asym_time'] = asym_times[1]
            else:
                params['asym_time'] = asym_times
        elif (keywords[0] == 'at_calc'):
            line = infile.readline()
            words = string.split(line)
            fdwell = float(words[1])
            dwell = "%f" % fdwell
            params['dwell'] = dwell
            if verbose:
                print "dwell %s" % (dwell)
            params['dwell'] = dwell
        elif (keywords[0] == 'navigator'):
            line = infile.readline()
            words = string.split(line)
            if words[1] == '"y"':
                lcnav = 1
            ...
            if verbose:
                print "quiet_interval %s" % (quiet_interval)
            params['quiet_interval'] = quiet_interval
        elif (keywords[0] == 'gain'):
            ...
            if verbose:
                print "np %s" % (np)
            params['np'] = np
        elif (keywords[0] == 'nshots'):
            # Number of phase encodes.
            line = infile.readline()
            words = string.split(line)
            nshots = words[1]
            if verbose:
                print "nshots %s" % (nshots)
...
#               Leon's "spare" sequence is really the EPI sequence with delay.
                pulse_sequence = "epi" 
            if verbose:
                print "pulse_sequence %s" % (pulse_sequence)
            ...
            nvol_images = words[0]
            if verbose:
                print "nvol_images %s" % (nvol_images)
    ...
    if pulse_sequence == 'mp_flash3d':
        ...
        if ns > 1:
            ...
            gap = 0
    params['gap'] = gap
    params['flash_converted'] = flash_converted
