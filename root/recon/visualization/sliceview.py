...
class sliceview (gtk.Window):
    ...
    def radioHandler(self, button, transform):
        ...
        if self.orient_mode >= 0:
            ...
            mat = (mat[::-1])[:,::-1]
            self.transform = compose_xform(mat, prev_xform=transform)
        else:
    ...
    def connectFigureCanvasEvents(self, mode="enable"):
        if mode == "init":
            # once enabled, these four never get disconnected
    ...
    def save_movie(self, axis):
        ...
        wpix, hpix = round(scale*px), round(scale*py)
        cmap = self.sliceplot.getImage().cmap
        interp = self.sliceplot.getImage()._interpolation
        figdpi = 100.
    ...
    def orient_handler(self, action, current):
        ...
        if self.orient_mode >= 0:
            ...
            T = (T[::-1])[:,::-1]
            self.transform = compose_xform(T, prev_xform=prefilter)
        else:
            new_zyx = [-3, -2, -1]
    ...
    def slice_proportions(self):
        im = self.data
        dim_lens = N.concatenate(((1,)*(3-im.ndim), N.array(im.shape[-3:])))
        dimsizes = im.dr * dim_lens
