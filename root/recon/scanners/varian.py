...
import numpy as np
from recon.util import qmult, eulerRot, Quaternion, normalize_angle, checkerline
...
class ProcParImageMixin (object):
    ...
    def slice_positions(self):
        ...
        return 10. * np.asarray(self._procpar.pss)\
    #-------------------------------------------------------------------------
    @CachedReadOnlyProperty
    def gss(self):
...
##         return self.pulse_sequence == 'mp_flash3d' and \
    ...
    @CachedReadOnlyProperty
    def raw_dtype(self):
        return self._procpar.dp[0] == 'y' and np.int32 or np.int16
    #-------------------------------------------------------------------------
    @CachedReadOnlyProperty
    ...
    def echo_time(self):
        if self.petable_name.find("alt") != -1:
            ...
            return self._procpar.te[0] - \
                   np.floor(self.pe_per_seg)/2.0*self.echo_spacing        
    #-------------------------------------------------------------------------
    @CachedReadOnlyProperty
    ...
    @CachedReadOnlyProperty
    def pe_times(self):
        return np.asarray([self.echo_time + pe*self.echo_spacing
...
class FidImage (ScannerImage, ProcParImageMixin):
    ...
    """
    #-------------------------------------------------------------------------
    def __init__(self, filestem, vrange=None, target_dtype=np.complex64):
        ...
        if ref_path is not filestem and path.exists(ref_path+".fid"):
            ...
            self.ref_data = np.array([self.ref_data[0], ref_fid.ref_data[0]])
    ...
    def initializeData(self):
        ...
        self.data = np.zeros((self.n_vol, self.n_slice,
                              self.n_pe_true, self.n_fe_true), np.complex64)
        self.nav_data = np.zeros((self.n_vol, self.n_slice,
                                  self.nav_per_slice, self.n_fe_true),
                                 np.complex64)
        self.ref_data = np.zeros((nrefs, self.n_slice,
                                  self.n_pe_true, self.n_fe_true), np.complex64)
        self.ref_nav_data = np.zeros((nrefs, self.n_slice,
                                      self.nav_per_slice, self.n_fe_true),
                                     np.complex64)
    ...
    def epi_trajectory(self, pe0=None):
        ...
        if self.sampstyle == "centric":
            ...
            b = np.floor((np.arange(float(M))+pe0)/float(self.nseg)).astype(np.int32)
        aref = a
        return (a, b, n2, aref)
    ...
    def seg_slicing(self, n):
        ...
        seg_rows = self.petable[n*pe_per_seg:(n+1)*pe_per_seg].copy()
        seg_rows.sort()
        return seg_rows
    ...
    def realizeOrientation(self):
        ...
        # This is the orientation model for Varian data...
        # the axis system seems to be right handed in the sense that
        # {+X,+Y,+Z} natively maps to {L,A,I} (or {-R,A,-S}), which is a
        # simple rotation to {R,A,S}, the neurological orientation. The 
        # trick here is to find out how the {i,j,k} basis is rotated within
        # the scanner's frame. The inverse of this rotation is the mapping
        # from {i,j,k} to {L,A,I} (which is then a simple mapping to {R,A,S})
        
        phi,psi,theta = self.phi, self.psi, self.theta
        ang = np.array([self.phi, self.psi, self.theta])
        ang = (ang/180.)*np.pi
        phi, psi, theta = ang.tolist()
        # find out the (closest) image plane
        if theta == 0 and psi == 0:
            self.image_plane = "axial"
        elif abs(theta) == np.pi/2 and psi == 0:
            self.image_plane = "coronal"
        elif abs(theta) == np.pi/2 and abs(psi) == np.pi/2:
            self.image_plane = "sagittal"
        else:
            self.image_plane = "oblique"

        # From the Varian Imaging Manual:
        # Two of these Euler angle parameters, psi and theta, determine the
        # orientation of a vector that is perpendicular to the desired imaging
        # plane (the slice position pss specifies the distance from the origin
        # to the imaging plane along this vector). Theta is the angle between
        # this vector and the magnet Z axis. Psi is the angle between the
        # projection of this vector onto the magnet XY plane, and the magnet
        # Y axis. The third angle, phi, determines the rotational orientation
        # of the imaging plane about the psi-theta vector. 

        # these are the ijk vectors in an unrotated frame:
        # i -- readout is from pos-to-neg Y (anterior to posterior)
        # j -- phase encode is from neg-to-pos X (right-to-left)
        # k -- slicing is from neg-to-pos z (superior to inferior)
        ijk0 = np.array([ [0, -1, 0], [1, 0, 0], [0, 0, 1] ]).T
        # the steps for rotation are
        # 1) theta degrees cw about +x
        # 2) psi degrees cw about +z
        rx = eulerRot(theta=theta)
        rz = eulerRot(phi=psi)
        ijkp = np.dot(rz, np.dot(rx, ijk0))
        # then there is an in-plane rotation of phi degrees about k-prime
        kp = ijkp[:,2]
        (x,y,z) = kp
        # this from http://en.wikipedia.org/wiki/Rotation_matrix
        rk = np.array([ [0, -z, y], [z, 0, -x], [-y, x, 0] ])*np.sin(phi) + \
             (np.eye(3) - np.outer(kp, kp))*np.cos(phi) + np.outer(kp, kp)
        ijk_rot = np.dot(rk, ijkp)
        # The columns of ijk_rot are the x,y,z components of the final i,j,k
        # frame. So from a coordinate frame where the (i',j',k') vectors are
        # the identity basis, ijk_rot is the mapping to lab coordinates (x,y,z).        # Lastly the mapping between from x,y,z to R,A,S is known for a
        # head-into-bore, suppine position--(R,A,S) = (-X,Y,-Z):
        ras_rot = np.array([ [-1, 0, 0],
                             [ 0, 1, 0],
                             [ 0, 0,-1] ])
        self.orientation_xform = Quaternion(M=np.dot(ras_rot, ijk_rot))
            
    #-------------------------------------------------------------------------
    def _load_petable(self):
        """
        Read the phase-encode table and save it to record the mapping from
        acquisition order to PE row position.
    ...
    def _read_compressed_volume(self, fidfile, vol):
        ...
        bias = np.array([hdr.lvl + 1.j*hdr.tlt], np.complex64)
        volume = np.empty((self.n_slice*self.n_pe*self.n_fe_true), np.complex64)
...
##         volume = np.empty(
##             (self.n_slice, self.nseg, self.pe_per_seg, self.n_fe_true),np.complex64)
...
##                     volume[sl,seg,self.nav_per_seg+pe].imag[:] = data['imag']
##         return np.reshape(volume, (self.n_slice, self.n_pe, self.n_fe_true))
    ...
    def _read_asems_nccnn_volume(self, fidfile, vol):
        ...
        @return: block of data with shape (n_slice*n_pe, n_fe_true)
        """
        volume = np.empty((self.n_pe,self.n_slice*self.n_fe_true), np.complex64)
        ...
        bias = np.array( [[h.lvl, h.tlt] for h in hdr], np.float32).transpose()
    ...
    def _read_asems_nscsn_volume(self, fidfile, vol):
        ...
        offset = np.repeat(np.arange(self.n_pe), self.n_slice) * \
                 (self.n_vol_true*self.n_slice) + vol*self.n_slice
        s_idx = np.array( range(self.n_slice) * self.n_pe )
        vol_sl = s_idx + offset
        (hdr, data) = fidfile[vol_sl]
        bias = np.array( [[h.lvl, h.tlt] for h in hdr], np.float32).transpose()
...
##             return "epi2fid"
    ...
    def loadData(self, datadir):
        ...
        #time_rev = n_fe_true - 1 - np.arange(n_fe_true)
        ...
        for vnum, vol in enumerate(self.ref_vols+self.image_vols):
            ...
            if time_reverse:
                volume.shape = (n_slice, nseg, n_pe/nseg, n_fe_true)
                revtmp = volume[:,:,1::2,::-1].copy()
                volume[:,:,1::2,:] = revtmp
                volume.shape = (n_slice, n_pe, n_fe_true)
            
            # strip off data into appropriate arrays
            # if nav_per_seg, navigator line(s) is the 1st in a seg
            if nav_per_seg:
                nav_lines = []
                for nav in range(nav_per_seg):
                    nav_lines += range(nav, n_pe, n_pe/self.nseg)
                nav_lines.sort()
            ...
            if needs_pe_reordering:
                if self.isravi:
                    # 1st nrow/nseg of vol is all slices' 1st seg
                    # assuming the pattern follows for nseg > 2??
                    nrow = n_slice * n_pe_true
                    volume.shape = (nrow, n_fe_true)
                    for seg in range(nseg):
                        dseg = volume[seg*nrow/nseg:(seg+1)*nrow/nseg]
                        dseg.shape = (n_slice, n_pe_true/nseg, n_fe_true)
                        seg_rows = self.petable[seg*n_pe_true/nseg:(seg+1)*n_pe_true/nseg]
                        seg_slice = (slice(None), seg_rows, slice(None))
                        data[vidx][seg_slice] = dseg
                    # do something about nav data here
                    if nav_per_seg:
                        nav_blk = navdata[vidx].copy()
                        nrow = n_slice * nav_per_seg * nseg
                        nav_blk.shape = (nrow, n_fe_true)
                        for seg in range(nseg):
                            d_slice = (vidx, slice(None),
                                       slice(seg*nav_per_seg,
                                             (seg+1)*nav_per_seg),
                                       slice(None))
                            b_slice = (slice(seg*nrow/nseg, (seg+1)*nrow/nseg),
                                       slice(None))
                            navdata[d_slice].flat[:] = nav_blk[b_slice].flat[:]
                        del nav_blk
                else:
                    data[vidx][:,self.petable,:] = volume
            else:
                data[vidx] = volume
...
class FidFile (_HeaderBase):
    ...
    def __init__( self, name ):
        ...
        dt = self.ebytes == 2 and np.int16 or np.int32
        if self.ebytes == 2:
            elem_dtype = np.dtype(('>u4',
                                   {'real':('>i2',0), 'imag':('>i2',2)}))
        else:
            elem_dtype = np.dtype(('>S8',
                                   {'real':('>i4',0), 'imag':('>i4',4)}))
        
        block_header_size = self.bbytes - block_data_size
        #hdr_dtype = np.dtype('S%d'%block_header_size)
        hdr_dtype = np.dtype('>V%d'%block_header_size)
        blk_dtype = np.dtype((elem_dtype, num_elem))
        #blk_dtype = np.dtype((elem_dtype, (self.ntraces,num_elem/self.ntraces)))
        dat_dtype = np.dtype({'names':['hdr','data'],
                              'formats':[hdr_dtype,blk_dtype]})
        self.mmap = np.memmap(name, dtype=dat_dtype, offset=self.header_size(),
...
class FDFImage (ReconImage, ProcParImageMixin):
    ...
    def loadData(self):
        ...
        for volnum in self.image_vols:
            ...
            volumes.append(np.asarray(slices))
        self.setData(np.asarray(volumes))
...
class FDFFile (object):
    ...
    def loadData(self):
        ...
        self.data = np.fromstring(self.infile.read(),
