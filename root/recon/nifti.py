...
class NiftiReader:
    ...
    def toImage(self):
        ...
        return ReconImage(self.data, self.isize, self.jsize,
...
class NiftiWriter (object):
    ...
    def write(self, filestem):
        ...
        if self.filetype=="single":
            ...
            file(fname,'wb').write(self.make_hdr())
            #write extension? 
            file(fname,'ab').write(default_extension)
            file(fname,'ab').write(self._dataview(self.image[:]).tostring())
        else:
            headername, imagename = "%s.hdr"%filestem, "%s.img"%filestem
            file(headername,'wb').write(self.make_hdr())
            file(imagename, 'wb').write(self._dataview(self.image[:]).tostring())
