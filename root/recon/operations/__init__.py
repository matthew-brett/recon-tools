...
from recon import __version__, import_from
...
class Operation (object):
    ...
    def opHelp(self):
        ...
        for p in self.params:
            print "%s: %s (Default value: %s)"%(p.name, p.description,
                                              str(p.default))
...
#-----------------------------------------------------------------------------
def ChannelAwareOperation(fn):
    """
    A decorator that runs data through an operation that might
    need to cross channels.
    """
    def newrun(op, image):
        op.haschans = hasattr(image, 'n_chan')
        fn(op, image)
    return newrun
#-----------------------------------------------------------------------------
def ChannelIndependentOperation(fn):
    """
    A decorator that runs data through a multi-channel oblivious operation,
    channel by channel if necessary.
    """
    def newrun(op, image):
        try:
            run_combined = op.SOS and image.combined
        except:
            run_combined = False
        if hasattr(image, 'n_chan') and not run_combined:
            for c in range(image.n_chan):
                image.load_chan(c)
                print "running channel", c
                fn(op, image)
...
class RunLogger (object):
    ...
    def __init__(self, ostream=sys.stdout):
        ...
        print >> self.ostream, "#Data processed by Recon Tools v. "+__version__
